...
Arg *OptTable::ParseOneArg(const ArgList &Args, unsigned &Index,
  ...
  std::cout << "argparse 9" << std::endl;
  unsigned Prev = Index;
  const char *Str = Args.getArgString(Index);

  std::cout << "argparse 10" << std::endl;
  ...
  std::cout << "argparse 13" << std::endl;
  const Info *End = OptionInfos.data() + OptionInfos.size();
  std::cout << "argparse 14" << std::endl;
  StringRef Name = StringRef(Str).ltrim(PrefixChars);
  std::cout << "argparse 15" << std::endl;

  // Search for the first next option which could be a prefix.
  Start = std::lower_bound(Start, End, Name.data());
  std::cout << "argparse 16" << std::endl;
  ...
  for (; Start != End; ++Start) {
    std::cout << "argparse 17" << std::endl;
    unsigned ArgSize = 0;
    // Scan for first option which is a proper prefix.
    for (; Start != End; ++Start) {
      std::cout << "argparse 18" << std::endl;
      if ((ArgSize = matchOption(Start, Str, IgnoreCase))) {
        std::cout << "argparse 19" << std::endl;
        break;
      }
    }
    if (Start == End)
    ...
    std::cout << "argparse 20" << std::endl;
    Option Opt(Start, this);

    std::cout << "argparse 21" << std::endl;
    if (FlagsToInclude && !Opt.hasFlag(FlagsToInclude)) {
      std::cout << "argparse 22" << std::endl;
      continue;
    }

    std::cout << "argparse 23" << std::endl;
    if (Opt.hasFlag(FlagsToExclude)) {
      std::cout << "argparse 24" << std::endl;
      continue;
    }

    std::cout << "argparse 25" << std::endl;
    // See if this option matches.
    if (Arg *A = Opt.accept(Args, Index, ArgSize)) {
      std::cout << "argparse 26" << std::endl;
      return A;
    }

    std::cout << "argparse 27" << std::endl;
    // Otherwise, see if this argument was missing values.
    if (Prev != Index) {
      std::cout << "argparse 28" << std::endl;
      return nullptr;
    }

    std::cout << "argparse 29" << std::endl;
  }

  std::cout << "argparse 30" << std::endl;
  // If we failed to find an option and this arg started with /, then it's
  // probably an input path.
  if (Str[0] == '/') {
    std::cout << "argparse 31" << std::endl;
    return new Arg(getOption(TheInputOptionID), Str, Index++, Str);
  }

  std::cout << "argparse 32" << std::endl;
...
InputArgList OptTable::ParseArgs(ArrayRef<const char *> ArgArr,
                                 ...
                                 unsigned FlagsToInclude,
                                 unsigned FlagsToExclude) const {
  std::cout << "argparse 1" << std::endl;
  InputArgList Args(ArgArr.begin(), ArgArr.end());

  std::cout << "argparse 2" << std::endl;
  // FIXME: Handle '@' args (or at least error on them).

  MissingArgIndex = MissingArgCount = 0;
  unsigned Index = 0, End = ArgArr.size();
  while (Index < End) {
    std::cout << "argparse 3" << std::endl;
    // Ingore nullptrs, they are response file's EOL markers
    if (Args.getArgString(Index) == nullptr) {
      std::cout << "argparse 4" << std::endl;
      ++Index;
      continue;
    }
    std::cout << "argparse 5" << std::endl;
    // Ignore empty arguments (other things may still take them as arguments).
    StringRef Str = Args.getArgString(Index);
    std::cout << "argparse 6 - " << Str.str() << std::endl;
    if (Str == "") {
      std::cout << "argparse 7" << std::endl;
      ++Index;
      continue;
    }

    std::cout << "argparse 8" << std::endl;
    unsigned Prev = Index;
